...
class Configuration:
    ...
    def __init__(self) -> None:
        self.load_env()
        self.api_key = os.getenv("xyz")  # or use another key variable
            ...
            try:
                if self.session:
                    try:
                        ...
                        logging.info(f"Session cleanup cancelled for {self.name}")
                    except Exception as e:
                ...
                if self.stdio_context:
                    try:
                        ...
                        logging.info(f"Stdio context cleanup cancelled for {self.name}")
                ...
                logging.info(f"Cleanup cancelled for server {self.name}")
            except Exception as e:
...
class ChatSession:
        ...
        if cleanup_tasks:
            ...
            for res in results:
                if isinstance(res, asyncio.CancelledError):
                    logging.info("Cleanup cancelled gracefully.")
    ...
    async def process_prompt(self, user_prompt: str) -> str:
        ...
        # Initialize servers if not already initialized
        for server in self.servers:
            await server.initialize()

        # List and format tools for the system prompt
        all_tools = []
        for server in self.servers:
            tools = await server.list_tools()
...
async def main() -> None:
    """Initialize and run the chat session with a test prompt."""
    ...
    # Replace with your desired prompt string
    test_prompt = "What is the weather like in New York?"
    response = await chat_session.process_prompt(test_prompt)
